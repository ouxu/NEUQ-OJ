...
class ProblemsTable extends React.Component {
    ...
    componentWillMount() {
        let page=sessionStorage.getItem("neuq_oj.problempagecurr")||1;
        let size= sessionStorage.getItem("neuq_oj.problempagesize")||20;
    ...
    onSeacrch(){
        ...
        if (searchText.length<1){
            let page=1;
            let size= sessionStorage.getItem("neuq_oj.problempagesize");
            this.props.getProblemTable(page,size);
        }else{
                ...
                (<span>
                    <span >{//标签渲染
                        record.tags?(record.tags.map((value,index)=>{
                            return <Tag color="#87d068" style={{Padding: 0}} key={index+400} className="problem-title-tags">{value.tag_title}</Tag>
                        })):null}</span>
