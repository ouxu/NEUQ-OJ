  ...
  render() {
    ...
    const columns = [{
      ...
      render: (record) => {
        ...
        if (status === 'Y') { return accepted.Y; } else if (status === 'N') { return accepted.N; }
        return null;
      },
      width: 40,
      key: 'problem-status',
      ...
      render: record =>
                (<span>
                  <span >{// 标签渲染
                        record.tags ? (record.tags.map((value, index) => <Tag color="#87d068" style={{ Padding: 0 }} key={index + 400} className="problem-title-tags">{value.tag_title}</Tag>)) : null}</span>
